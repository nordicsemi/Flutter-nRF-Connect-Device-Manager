...
class SettingsManager(
    ...
    fun writeSetting(key: String, value: Any, result: MethodChannel.Result) {
        ...
        val writeCallback = object : McuMgrCallback<McuMgrResponse> {
            override fun onResponse(response: McuMgrResponse) {
                try {
                    if (response.isSuccess) {
                        result.success(response.bytes)
        ...
        if (useByteStringEncoding) {
            // Byte string encoding: value → byte[] → CBOR bstr
            ...
            mcuMgrSettingsManager.write(key, valueBytes, writeCallback)
        } else {
